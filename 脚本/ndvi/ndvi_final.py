...
策略: Landsat C2 L2, 100m 输出, 小窗口快速读取
  - 输出分辨率: 0.001° (~100m), 2738x2660 像素
  - 场景读取窗口: max 800px (利用 COG overview 加速)
  - 按 path 分组选景, 确保全城覆盖
  - 生长季均值合成 (6-9月)
  - 1990用Landsat-5, 2000+用Landsat-5/7/8/9
"""
...
def process_scene(item):
    """单景 Landsat → NDVI → reproject (小窗口, 快速)"""
...
for year in YEARS:
    ...
    if year != 1990 and os.path.exists(out_path) and os.path.getsize(out_path) > 500 * 1024:
        print(f"[{year}] 已存在，跳过\n")
        continue
    ...
    selected = []
    per_path = 20 if year == 1990 else 10
    total_limit = 100 if year == 1990 else 60
