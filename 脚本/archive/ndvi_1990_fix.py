...
print(f"输出网格: {OUT_WIDTH}x{OUT_HEIGHT}")
...
print(f"{'='*60}")
print(f"[1990] 扩展搜索 Landsat-5...")

# 策略1: 生长季 + 云量80%
items = search_mpc(SEARCH_BBOX, "1990-04-01/1990-10-31", ["landsat-5"], 80, 500)
print(f"  4-10月,云量<80%: {len(items)} 景")

# 策略2: 全年 + 放宽云量
items2 = search_mpc(SEARCH_BBOX, "1990-01-01/1990-12-31", ["landsat-5"], 100, 500)
...
# 选景: 云量≤70%, 每path最多25景, 总共最多75景
selected = []
PER_PATH = 25
for pp, grp in path_groups.items():
    grp_filtered = [s for s in grp if s.properties.get('eo:cloud_cover', 100) <= 70]
    if len(grp_filtered) < 5:  # 如果低云量场景太少，放宽到90%
        grp_filtered = [s for s in grp if s.properties.get('eo:cloud_cover', 100) <= 90]
    grp_filtered.sort(key=lambda x: x.properties.get('eo:cloud_cover', 100))
    selected.extend(grp_filtered[:PER_PATH])
...
if clouds:
    print(f"  云量范围: {min(clouds):.1f}%~{max(clouds):.1f}%, 平均{np.mean(clouds):.1f}%")
